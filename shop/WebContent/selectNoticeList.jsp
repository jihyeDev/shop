...
%>
	<div class="container pt-3">
		...
		<h1 class="jumbotron bg-white text-center">공지사항</h1>
		<table class="table table-sm">
			<thead style="text-align:center;">
				<tr>
					<th style="width:5%; text-align:center;">NO</th>
					<th>TITLE</th>
					<th style="width:20%; text-align:center;">CREATE</th>
				</tr>
			</thead>
			<tbody>
				<%
			...
			if(currentPage != 1){
			%>
				<a href="<%=request.getContextPath()%>/selectNoticeList.jsp?currentPage=<%=1%>&searchNoticeTitle=<%=searchNoticeTitle%>" class="btn btn-outline-secondary center-block">◀처음</a>
			<%
			}
			...
			%>
				<a href="<%=request.getContextPath()%>/selectNoticeList.jsp?currentPage=<%=startPage-displayPage%>&searchNoticeTitle=<%=searchNoticeTitle%>" class="btn btn-outline-secondary">&lt;이전</a>
			<%
			}
			...
			for(int i=startPage; i<=endPage; i++){
				if(currentPage == i){
			%>
					<a href="<%=request.getContextPath()%>/selectNoticeList.jsp?currentPage=<%=i%>&searchNoticeTitle=<%=searchNoticeTitle%>" class="btn btn-secondary"><%=i%></a>
			<%
				} else if(endPage<lastPage || endPage == lastPage){
			%>
					<a href="<%=request.getContextPath()%>/selectNoticeList.jsp?currentPage=<%=i%>&searchNoticeTitle=<%=searchNoticeTitle%>" class="btn btn-outline-secondary"><%=i%></a>
			<%	
			...
			if(endPage < lastPage){
			%>
				<a href="<%=request.getContextPath()%>/selectNoticeList.jsp?currentPage=<%=startPage+displayPage%>&searchNoticeTitle=<%=searchNoticeTitle%>" class="btn btn-outline-secondary">다음></a>
			<%
			}
			...
			%>
				<a href="<%=request.getContextPath()%>/selectNoticeList.jsp?currentPage=<%=lastPage%>&searchNoticeTitle=<%=searchNoticeTitle%>" class="btn btn-outline-secondary">끝▶</a>
			<%
			}
			%>
		...
		<div class="float-right">
			<form action="<%=request.getContextPath()%>/selectNoticeList.jsp" method="get">
